...
def process_words(process_settings: ProcessSettings):
    ...
    pending_process: list[SourceContent] = [
        f
        for f in file_manager.source_content
        if f.get_name() in process_settings.inputs
    ]

    for sc in pending_process:
        logger.debug(f"Processing file: {sc.get_name()}")
        short_dict = _process_input(
            sc,
            process_settings.word_check,
            process_settings.freq_min,
            process_settings.requires_definition,
            process_settings.min_word_length,
        )
...
def _process_input(
    ...
    short_dict = word_service.ask_user(content_dict) if word_check else content_dict
...
def _analyze_content(
    ...
    word_freq = defaultdict(lambda: {"frequency": 0, "definition": None, "content": []})
    for c in content:
        for word in subtitle_service.get_base_words(c.sentence):
            ...
            if len(word) >= min_word_length and word not in ignore_list:
                ...
                if word_freq[word]["definition"] is None:
                    ...
                    word_freq[word]["definition"] = sd.to_dict() if sd else False
                word_freq[word]["content"].append(c)

    filtered_word_freq = {
        w: word_freq[w]
        for w in word_freq
        if word_freq[w]["frequency"] >= freq_min
        and (not requires_definition or bool(word_freq[w]["definition"]))
    }
    sorted_word_freq = dict(
        sorted(
            filtered_word_freq.items(),
