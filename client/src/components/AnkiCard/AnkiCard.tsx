...
const AnkiCard: FunctionComponent<AnkiCardProps> = ({ onCancel }) => {
  ...
  const [configFieldsSelected, setConfigFieldsSelected] = useState<
  ...
  useEffect(() => {
    ...
    const fetchDecks = async () => {
      try {
        ...
        const deckMap: [string, number][] = Object.entries(
        ...
        const modelMap: [string, number][] = Object.entries(
  ...
  useEffect(() => {
    const fetchModelFieldsAndSettings = async () => {
      if (modelSelected && selectedModelId) {
        try {
          const ankiResp = await axios.get(
            `/api/anki/models/${selectedModelId}/fields`,
          );
    ...
    <div className="card">
      ...
      {!modelSelected ? (
        <>
          ...
          <button onClick={handleModelSelect}>Select Model</button>
        </>
      ) : (
        <>
          <div className={commonStyles.configContainer}>
            ...
            <div className={commonStyles.configScrollable}>
              {modelFields.map((field, index) => (
                <div className={commonStyles.configRow} key={index}>
                  <label className={commonStyles.configLabel}>{field}</label>
